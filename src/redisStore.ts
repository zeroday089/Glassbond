import type { AuditEvent, BanRecord, GlassBondStore, PathActivity, RedisLikeClient, SecurityMetrics, StoreCounter, TokenBucketResult } from "./types.js";
...
export class RedisStore implements GlassBondStore {
  ...
  async recordEvent(event: AuditEvent): Promise<void> {
    await this.fallback.recordEvent(event);
  }

  async getEvents(limit?: number): Promise<AuditEvent[]> {
    return this.fallback.getEvents(limit);
  }
