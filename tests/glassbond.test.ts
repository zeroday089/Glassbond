...
describe("GlassBond", () => {
  ...
  it("registers as a Fastify plugin", async () => {
    type Hook = (request: { raw: IncomingMessage }, reply: { code: (status: number) => unknown; header: (name: string, value: string) => unknown; send: (payload: unknown) => void }) => Promise<void> | void;
    let hook: Hook | undefined;
    const instance = { addHook: (_name: "onRequest", fn: Hook) => { hook = fn; } };
    const done = vi.fn();
    const middleware = glassbond({ threatThreshold: 50 });
    middleware(instance, {}, done);
    expect(done).toHaveBeenCalledOnce();
    const reply = { code: vi.fn(() => reply), header: vi.fn(() => reply), send: vi.fn() };
